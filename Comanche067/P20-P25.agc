...
PROG20		TC	BANKCALL
		...
		TC	UPFLAG		# SET TRACKFLAG ## FIXME
		ADRES	TRACKFLG	# BIT 5 FLAG 1 ## FIXME
...
R23CSM		CA	NEGONE
		...
		TC	+2		# PROC
		TC	-5		# ENTER
R23CSM1		CAF	V53		# PERFORM ALT LOS SIGHT MARK
		TC	BANKCALL
		CADR	GOMARK2
...
R61CSM		CAF	EBANK6		# SWITCH TO EBANK 6
		...
		EXIT
		TC	DOWNFLAG	# RESET FLAG TO INHIBIT FURTHER R60 ## FIXME
		ADRES	PDSPFLAG ## FIXME V50N18FL	#    ATTITUDE MANEUVERS
		INDEX	MPAC
		TC	+1
...
RANGERD		EXIT
		...
		CAF	OCT11
		EXTEND
		WOR	CHAN13		# GENERATE SHIFT PULSES TO RADR, SET R. BIT
		RELINT
		EXTEND
		DCA	TIME2
...
CRS61.2		VLOAD	VSU
		...
		SXA,1			# QUOT(MPAC) X CROSS(PUSH0)
			Q6111		# SAVE SCALE OF RESULT (R-15,1X).
					# X1= NORM OF QUOT. QUOT SCALE B7-B29=B-22
...
HOLDATT		VLOAD			# HOLD EXTRAPOLATED CSM POSITION AND
		...
		STOVL	SAVEPOS
			VATT
		STORE	SAVEVEL
CALCLEM		DLOAD			# EXTRAPOLATE LEM STATE VECTOR TO SAME
...
S22F244		SLOAD	BHIZ		# FIG 2.4-4	TEST OFF=0
			CXOFF
			S22BOX44
		SR			# SCALE OFFSET B-14 THEN TEST GR. 8NN
			9D
		STORE	ALPHAV		# TEMP
		SLOAD	DSU
			8NN
...
S22F2410	SETPD	VLOAD		# COMPUTE FORMULA 2.4.10
		...
		PUSH			# PD 2D 0D=COSA=-(UM.RC)/ABVAL(RC) 	B-1
		DSQ	BDSU
			DEC1B2
...
P23		TC	DOWNFLAG
		...
		SSP	CLEAR
			MARKINDX
			1
			TARG2FLG	# TARGET FLAG USED BY R52 AND R53
		CLEAR	SSP
			TARG1FLG
			STARIND
			0
