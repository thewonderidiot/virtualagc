...
##		2017-02-06 RSB	Proofed comment text using octopus/ProoferComments,
##				though the printout/scan quality is really too poor, 
##				since no corresponding file exists in any other AGC
##				version against which it can be diff'd, and since 
##				traditional proofing is so poor at catching errors.
...
P20FLGON	EXTEND
		...
		COUNT*	$$/P3136
P31		TC	P20FLGON	# SET TRACK,UPDATE, CLEAR FINAL FLAGS
		TC	INTPRET
P31ALMRT	CALL
			INPUTDSP
COMPMU		VLOAD	ABVAL
			RRECTCSM
		PUSH	PDVL*		# 			PL AT 04D
			MUTABLE,1	# RTX1=1/MU B+34,B+28, RTMU=SQRT(MU)
		STOVL	RTX1		# B-18,B-15, RTSR1/MU=1/SQRT(MU) 	B+17,B+14
			VRECTCSM
		VSQ	DMPR
			RTX1		# VSQ/MU				B+2 ,B+18
		DMP	SL3		# R VSQ/MU		PL AT 02D	B-6
		BDSU	SR1R
			D1/32		# ( 2 - R VSQ/MU )			B-7
		DDV	PUSH		# ( 2 - R VSQ/MU )/R	   AT 00D	B+22,B+20
		SQRT	BDDV		# 2 PI R/(2 - R VSQ/MU)			B-15,B-14
			2PISC
		DMP	PDDL		# 2PI R SQRT(MU)/(2MU-R VSQ)            B-2,B-0
			RTSR1/MU
		NORM	PDDL
			X1
		SL*	DDV
			0 -7D,1		# HAFPA1 = PERIOD/2			B-28
		BDSU			# TIG = ( TCSI - HAFPA1 )		B-28
			TCSI
		STORE	TIG		# TIG = ( TCSI - PERIOD/2 )		B-28
		EXIT
		CA	V06N33
		TC	VNFLASH
		TC	INTPRET
...
P31RT		VLOAD
		...
		SL	CALL
			...
			N90/N81
		CALL			#		 X
			VN1645		# S32/33.1, DISPLAYS AND COMPUTES DELVLVC
		GOTO			# AND DELVSIN.
			P31RECYC
25THOUS		2DEC*	-.4114470842 E-5 B+17*	#				B+17
...
# PLANE CHANGE TARGETING PROGRAM	P36
# 
# PURPOSE
...
#	DELVSIN		DELTA VELOCITY AT PC - REFERENCE
		...
		COUNT*	$$/P3136
P36		TC	P20FLGON	# SET TRACK,UPDATE - CLEAR PC FLAGS
		TC	UPFLAG
		ADRES	PCFLAG
...
P36A		SET	DLOAD
		...
		STOVL	VVEC
		STADR
		STCALL	RVEC		# COMPUTE TRANSFER TIME OF 90 DEG
			TIMETHET
		DAD
			TCSI
		STORE	TIG
...
P36RECYC	VLOAD	CALL
			ZEROVECS
			N90/N81		# DISPLAY N90,N81 COMPUTE YDOT
		CALL
			VN1645
		GOTO
			P36RECYC
...
# ...... N90/N81 ....... COMPUTES YDOT, DISPLAYS N90 AND N81 FOR P31, P36
		COUNT*	$$/P3136
N90/N81		STQ
			N90RET
		STODL	DELVLVC
