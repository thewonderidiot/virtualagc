...
# FOR INTEGRVS, THE RCV,VCV, TET OF THE TEMPORARY STATE VECTOR MUST BE SET
#
...
ATOPCSM		STQ	RTB
		...
		GOTO
			S2
MOVEACSM	TC	SETBANK
 +1		TS	DIFEQCNT	# INITIALIZE INDEX
		INDEX	DIFEQCNT
		CA	RRECT
		INDEX	DIFEQCNT
		TS	RRECTCSM
...
NORFINAL	DMP	SR1R
		...
		DMP	SR
			.3D
			7		# DT IS TRUNCATED TO A MULTIPLE
		DLOAD	SL
			MPAC
			15D		#	OF 128 CSECS.
		PUSH	BOV
...
INTWAKE		CS	FLGWRD10	# IS THIS INTSTALLED ROUTINE TO BE
...
FORTYONE	DEC	41
		CS	PRIO30
		MASK	FLGWRD10
		TS	FLGWRD10	# RELEASE STALL AREA
		RELINT
		TCF	GOBAC
...
SETCOAST	AXT,2	CALL		# NOW MOVE PROPERLY SCALED RN,VN AND
			0		# PIPTIME TO INTEGRATION ERASABLES.
			INTSTALL
		CLEAR	VLOAD
			VINTFLAG
			RN
...
INTWAKEU	TC	INTPRET
		...
		CALL			# STORE VRECT INTO VCV AND ZERO OUT
			TINIRECT	# TDELTAV(6),TNUV(6),TC(2) AND XKEP(2)
